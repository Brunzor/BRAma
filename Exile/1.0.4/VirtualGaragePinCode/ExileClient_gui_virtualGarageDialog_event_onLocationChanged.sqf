...
private["_display", "_dropdown", "_listbox", "_confirmButton", "_nicknameBox", "_index", "_flag", "_storedVehicles", "_nickname", "_nearVehicles","_pinCode"];
...
switch (_index) do
{
	case 0:
	{
		...
		{
			{
                if (profileNameSpace getVariable ["ExileStreamFriendlyUI", false]) then {_pinCode = "";} else {_pinCode = " - Pin Code: " + (_x select 2);};				
                _nickname = _x select 1;                
				_index = _listbox lbAdd _nickname + _pinCode;
